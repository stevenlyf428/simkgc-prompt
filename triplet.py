...
class LinkGraph:
    ...
    def __init__(self, train_path: str):
        ...
        # 根据头id，存储上下文三元组 head_id -> ( (r,h) )
        self.prompt = {}
        examples = json.load(open(train_path, 'r', encoding='utf-8'))
        for ex in examples:
            head_id, r ,tail_id = ex['head_id'], ex['relation'] ,ex['tail_id']
            # head = get_entity_by_id(head_id)
            # tail = get_entity_by_id(tail_id)
            # triple = head + r + tail
            
            if head_id not in self.prompt:
                self.prompt[head_id] = set()
            self.prompt[head_id].add((head_id,r,tail_id))
            if tail_id not in self.prompt:
                self.prompt[tail_id] = set()
            self.prompt[tail_id].add((head_id,r,tail_id))
        logger.info('Done build context prompt with {} nodes'.format(len(self.prompt)))
        # 根据头id，存储上下文三元组 head_id -> ( (r,h) )
        # self.prompt = {}
        # examples = json.load(open(train_path, 'r', encoding='utf-8'))
        # for ex in examples:
    ...
    def get_neighbor_triple_prompt(self, entity_id: str, max_to_keep=5) -> List[str]:
        # make sure different calls return the same results
        neighbor_ids = self.prompt.get(entity_id, set())
        return sorted(list(neighbor_ids))[1:max_to_keep]
