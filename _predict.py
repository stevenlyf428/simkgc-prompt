...
class BertPredictor:
    ...
    # @torch.no_grad()
    # def predict_by_examples(self, examples: List[Example], entity_exs):
    #     data_loader = torch.utils.data.DataLoader(
    #         Dataset(path='', examples=examples, task=args.task),
    #         num_workers=1,
    #         batch_size=max(args.batch_size, 512),
    ...
    #     bs_item_ent_tr_list = []
    #     for idx, batch_dict in enumerate(data_loader):
    #         for relation_idx in batch_dict['relation2idx']:
    #             relation = relation_idx.item()
    #             bs_item_ent_tr_list.append(self.predict_by_entities(entity_exs, relation))
    #         # batch_size = len(batch_dict['inputs'])
    #         # for i in range(batch_size):
    #         #     bs_item_ent_tr_list.append(self.predict_by_entities(entity_exs,batch_dict['inputs'][i]['relation2idx'])) 
    #             # examples = []
    #             # for entity_ex in entity_exs:
    #             #     examples.append(Example(head_id='', relation=bs_iterm['relation2idx'],
    #             #                             tail_id=entity_ex.entity_id))
    #             # data_loader = torch.utils.data.DataLoader(
    #             #     Dataset(path='', examples=examples, task=args.task),
    #             #     num_workers=16,
    ...
    #     return torch.cat(hr_tensor_list, dim=0), torch.cat(tail_tensor_list, dim=0), torch.cat(bs_item_ent_tr_list,dim=0)

    # @torch.no_grad()
    # def predict_by_entities(self, entity_exs, relation) -> torch.tensor:
    #     examples = []
    #     for entity_ex in entity_exs:
    #         examples.append(Example(head_id='', relation=str(relation),
    #                                 tail_id=entity_ex.entity_id))
    #     data_loader = torch.utils.data.DataLoader(
    #         Dataset(path='', examples=examples, task=args.task),
    #         num_workers=2,
    ...
    #         outputs = self.model(**batch_dict)
    #         # ent_tensor_list.append(outputs['ent_vectors'])
    #         ent_tensor_list.append(outputs['ent_tr_vectors'])
